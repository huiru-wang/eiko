...
export function createReadTopicTool(topicRepo: TopicRepository): AgentTool<typeof ReadTopicParams> {
  return {
    name: "read_topic",
    label: "Read Topic",
    description: "获取指定 Topic 的完整详情，包括 title、summary、tags、bodyMarkdown、matchText 等。",
...
const CreateTopicParams = Type.Object({
  ...
  bodyMarkdown: Type.String({ description: "完整 Topic 正文（Markdown）" }),
  matchText: Type.Optional(Type.String({ description: "用于匹配的关键词文本" })),
});
...
const UpdateTopicParams = Type.Object({
  ...
  bodyMarkdown: Type.Optional(Type.String({ description: "更新后的完整 Topic 正文（Markdown）" })),
  matchText: Type.Optional(Type.String({ description: "用于匹配的关键词文本" })),
});
