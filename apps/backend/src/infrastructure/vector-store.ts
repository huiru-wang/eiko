...
export interface VectorStore {
  upsertRecord(record: { id: string; content: string; userId: string; occurredAt?: string }): Promise<void>;
  upsertTopic(topic: {
    id: string;
    title: string;
    summary: string;
...
export class SqliteVecVectorStore implements VectorStore {
  ...
  async upsertRecord(record: { id: string; content: string; userId: string; occurredAt?: string }): Promise<void> {
    ...
    await sql`
      insert into vec_records(record_id, user_id, embedding, occurred_at, embedding_text)
      values (${record.id}, ${record.userId}, ${JSON.stringify(embedding)}, ${record.occurredAt ?? new Date().toISOString()}, ${record.content})
    `.execute(this.db);
  ...
  async upsertTopic(topic: {
    ...
    await sql`
      ...
      values (${topic.id}, ${topic.userId}, ${JSON.stringify(embedding)}, ${topic.updatedAt ?? new Date().toISOString()}, ${embeddingText})
    `.execute(this.db);
...
export function buildTopicEmbeddingText(topic: {
  ...
  const bodyPreview = topic.bodyMarkdown.slice(0, 2000);
  return [
    `标题：${topic.title}`,
    topic.summary ? `摘要：${topic.summary}` : "",
    topic.tags.length > 0 ? `标签：${topic.tags.join(", ")}` : "",
    topic.matchText ? `匹配边界：${topic.matchText}` : "",
